...
use crate::minikv::parser::serializar;
use crate::minikv::storage::append_linea_log;
use crate::minikv::storage::reconstruir_estado;
use crate::minikv::storage::sobrescribir_data;
use crate::minikv::storage::vaciar_log;
...
impl FromStr for TipoComando {
    ...
    fn from_str(comando: &str) -> Result<Self, Self::Err> {
        match comando {
            ...
            _ => Err(ErrorCliente::UnknownCommand),
        }
    }
}
...
pub fn crear_comando(
    ...
    match (comando, clave, valor, extra) {
        ...
        | (TipoComando::Snapshot, Some(_), _, _) => Err(ErrorCliente::ExtraArgument),
        (TipoComando::Set, None, _, _) | (TipoComando::Get, None, _, _) => {
            Err(ErrorCliente::MissingArgument)
        }
        _ => Err(ErrorCliente::UnknownCommand),
    }
}
...
pub fn comando_set(clave: String, valor: String, path_log: &str, diccionario: &mut HashMap<String, String>) -> Result<(), ErrorServidorDatos> {
...
pub fn comando_unset(clave: String, path_log: &str,diccionario: &mut HashMap<String, String>) -> Result<(), ErrorServidorDatos> {
    let clave_log = serializar(&clave);
    let mut linea_log: String = String::new();
    linea_log.push_str("set ");
    linea_log.push_str(&clave_log);
...
pub fn comando_get(clave: String, diccionario: &mut HashMap<String, String>) -> Result<String, ErrorCliente> {
    diccionario.get(&clave).cloned().ok_or(ErrorCliente::NotFound)
}
...
pub fn comando_length(diccionario: &mut HashMap<String, String>) -> usize {
    diccionario.len()
}
...
pub fn comando_snapshot(path_data: &str, path_log: &str) -> Result<(), ErrorServidorDatos> {
...
mod tests {
    ...
    #[test]
fn test_01_el_comando_set_agrega_una_clave_en_diccionario_y_archivo() {
    let path_log = ".test.comando_set01.minikv.log";
    let path_data = ".test.comando_set01.minikv.data";
    borrar_archivos(&[path_data, path_log]);

    let mut diccionario = reconstruir_estado(path_data, path_log).unwrap();
    ...
    assert!(diccionario.contains_key("clave1"));

    let contenido = std::fs::read_to_string(path_log).unwrap();
    assert!(contenido.contains("set \"clave1\" \"valor1\""));
    borrar_archivos(&[path_data, path_log]);
}
#[test]
fn test_02_el_comando_unset_elimina_una_clave_del_diccionario_y_registra_en_archivo() {
    let path_log = ".test.comando_unset01.minikv.log";
    let path_data = ".test.comando_unset01.minikv.data";
    borrar_archivos(&[path_data, path_log]);
    ...
    comando_unset("clave1".to_string(), path_log, &mut diccionario).unwrap();

    assert!(!diccionario.contains_key("clave1"));

    let contenido = std::fs::read_to_string(path_log).unwrap();
    println!("CONTENIDO LOG: {:?}", contenido);
    assert!(contenido.contains("set \"clave1\""));
    assert!(!contenido.contains("set \"clave1\" \"valor1\" \nset \"clave1\" \n"));
    borrar_archivos(&[path_data, path_log]);
}
#[test]
fn test_03_el_comando_get_devuelve_el_valor_correcto_o_error_not_found() {
    let path_log = ".test.comando_get01.minikv.log";
    let path_data = ".test.comando_get01.minikv.data";
    borrar_archivos(&[path_data, path_log]);
    ...
    let resultado = comando_get("clave1".to_string(), &mut diccionario).unwrap();
    assert_eq!(resultado, "valor1".to_string());

    let resultado_not_found = comando_get("clave2".to_string(), &mut diccionario);
    assert_eq!(resultado_not_found, Err(ErrorCliente::NotFound));

    borrar_archivos(&[path_data, path_log]);
}
#[test]
fn test_04_el_comando_length_devuelve_la_cantidad_correcta_de_claves() {
    let path_log = ".test.comando_length01.minikv.log";
    let path_data = ".test.comando_length01.minikv.data";
    borrar_archivos(&[path_data, path_log]);

    let mut diccionario = reconstruir_estado(path_data, path_log).unwrap();

    assert_eq!(comando_length(&mut diccionario), 0);
    ...
    assert_eq!(comando_length(&mut diccionario), 2);

    comando_unset("clave1".to_string(), path_log, &mut diccionario).unwrap();

    assert_eq!(comando_length(&mut diccionario), 1);
    ...
    #[test]
    fn test_05_integracion_set_sobrescritura() {
        let path_log = ".test.integracion02.minikv.log";
        let path_data = ".test.integracion02.minikv.data";
        borrar_archivos(&[path_data, path_log]);
        let mut diccionario = reconstruir_estado(path_data, path_log).unwrap();
        comando_set("clave1".to_string(), "valor1".to_string(), path_log,&mut diccionario).unwrap();
        comando_set("clave1".to_string(), "valor2".to_string(), path_log,&mut diccionario).unwrap();
